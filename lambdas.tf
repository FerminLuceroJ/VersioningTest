...
module "db_reset" {
  ...
  create_function = try(each.value.enable_db_reset, var.rds_aurora_defaults.enable_db_reset, false)
  ...
  environment_variables = { "SECRET_NAME" : try(each.value.secret.name, var.rds_aurora_defaults.secret.name, "rds-${local.common_name}-${each.key}") }
...
resource "aws_lambda_layer_version" "db_reset" {
  ...
  count = var.rds_aurora_parameters != {} ? 1 : 0
